...
export default function GreenPopUp({ message, label, onClose }: IsPopUpProps) {
    return (
        <motion.div className="bg-green-500 w-[30rem] h-[20rem] rounded-md p-4 shadow-lg z-50"
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
            transition={{
                type: "spring",
            ...
            <div className="flex flex-col justify-center items-center rounded-md bg-white w-full h-[15rem] p-4">
                ...
                <button
                    className="mt-4 bg-green-500 text-white py-2 px-4 rounded hover:bg-green-500 hover:shadow-lg"
                    onClick={onClose}
                >
                    Close
                </button>
